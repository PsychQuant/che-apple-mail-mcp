...
### Added
- **`sanitize_links` formal spec coverage + builder-layer wiring contract tests** ([#85](https://github.com/PsychQuant/che-apple-mail-mcp/issues/85)). Closes the two P2 verify-followup gaps from cluster A's #19 ship: (Gap B) `openspec/specs/message-composition/spec.md` now has a formal Requirement block "Markdown mode honors opt-in URL scheme allowlist via `sanitize_links`" with 5 scenarios codifying default-off passthrough, javascript: blocking, allowlist preservation (http/https/mailto/tel), no-op semantics in plain/html modes, and the end-to-end wiring contract — `grep` against the spec went from 0 hits to 16 sanitize-related lines; (Gap A) `MailControllerComposeTests` adds 4 wiring contract tests pinning the builder→renderer forwarding for each composing tool, exercising both arms (default-off passthrough + sanitize_links=true blocks javascript:). Verified by fault injection: dropping the `sanitizeLinks` arg from any `build*Script`'s internal `renderBody` call now causes test failure. Coverage scope: builder→renderer seam (where filtering happens). Controller→builder one-line forwarding and schema→handler param parsing remain uncovered without further refactoring (tracked as future hardening if desired).
