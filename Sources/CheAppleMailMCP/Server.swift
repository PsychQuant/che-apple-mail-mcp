...
class CheAppleMailMCPServer {
    ...
    static func defineTools() -> [Tool] {
        [
            ...
            Tool(
                ...
                description: "Check whether Accessibility (GUI-scripting) is granted via AXIsProcessTrusted(). Required for the #175 wrapper-free compose path (compose_email / create_draft use mailto + keystrokes so the body isn't wrapped in <blockquote type=\"cite\"> on mobile clients). Returns status plus steps to grant it. Separate grant from check_fda. If denied, GUI-based composing calls are refused before composition starts; use open_mailto for a zero-TCC compose window (no attachments; save or send manually). This does not check Automation permission or guarantee that later GUI steps succeed.",
                inputSchema: .object(["type": .string("object"), "properties": .object([:])])
            ),
            Tool(
            ...
            Tool(
                name: "list_drafts",
                description: "List all draft emails for one account. Each entry carries `subject` and the draft's current numeric `id` (message ROWID), accepted by update_draft.draft_id and delete_email.id. This is not a stable key: autosave or synchronization can replace the row even before the next call. Use a freshly listed id promptly and handle a stale-id refusal; for an unchanged exact subject that is unique in the same account scope, update_draft.subject_match is an alternative content selector, not a permanent identity. Resolves the account's real drafts mailbox via Mail's unified drafts mailbox (works with localized/provider-specific names like Gmail's [Gmail]/草稿 — no hardcoded 'Drafts' lookup, see #174).",
                inputSchema: .object([
                    "type": .string("object"),
                    "properties": .object([
            ...
            Tool(
                name: "create_draft",
                description: "Create a new draft email. Success leaves the compose window open for review, manual edits and attachments; autosave or synchronization may change the saved draft ROWID before a later call. No cross-resave stable selector is exposed: both ROWID and RFC Message-ID changed in a controlled Mail resave (#405); do not treat a draft id as permanent. Display-name recipients (Name <email>) in to, cc AND bcc ride the clean path on DRAFTS: each list carrying a display name is omitted from the mailto URL and filled through the compose window — the field is located by its AXIdentifier (Mail.toField / Mail.ccField / Mail.bccField), focused, pasted from the clipboard, Tab-committed, and its tokens read back (count + display names) before the save (#277 to, #404 cc/bcc; needs Accessibility). A hidden Bcc field is revealed via View ▸ Bcc Address Field and deliberately left visible — the result says bcc_field_revealed: true. After the save — only when cc or bcc carried a display name; a bare-address call gets no recipients_verified — the draft carrying this subject (newest id wins, #409: another same-subject draft can be selected) has its cc/bcc ADDRESSES re-read and compared with the request: the result says recipients_verified: true, or recipients_verified: false with a recipients_diff JSON object, or recipients_receipt: unavailable when the read could not run — the draft is kept in every case, never deleted. Recipient receipt reads stop on the first failure without retry; only successful NOTFOUND reads are polled (at most three reads with 0.4 seconds between reads). Caveat: that receipt scans every account's drafts mailbox through a cancellable osascript subprocess (#406); when it hits the 45 s deadline the result says unavailable (not \"not found\") and the draft is still saved. Caveat (#408): Mail.toField / Mail.ccField / Mail.bccField were observed on Mail for macOS 27 only. One macOS 27 live run covered a mixed named+bare multi-recipient Cc list with named To/Bcc; Contacts-renamed token behavior and macOS 13–26 field identifiers remain unverified. Field lookup/reveal failure paths include FILLFIELD and BCCREVEAL before the explicit save; GUI cleanup can still fail and leave a window or draft. The body always comes from Mail's own editor, never from an AppleScript body assignment — that assignment is what makes Mail wrap the whole letter in <blockquote type=\"cite\">, which the sender cannot see locally while Gmail and Outlook show it as quoted text (#304). If a preflight eligibility check refuses the call before GUI work starts, it FAILS with a named reason and an alternative and creates/sends nothing. The six named eligibility reasons are: format is not 'plain'; empty subject; Accessibility not granted (grant it, or use open_mailto — zero TCC, no attachments); from_address is not a bare addr-spec; an attachment path contains non-ASCII characters (create the draft without attachments and drag the file in, #220); a to/cc/bcc recipient carries a display name on a SEND — never on a draft, where display names are GUI-filled (#277/#404). If a To/Cc/Bcc list contains any display name, that entire list (including bare entries and Bcc addresses) is temporarily written to the system clipboard for GUI filling. Clipboard-reading apps or Universal Clipboard may obtain it depending on system settings. Restoring prior clipboard contents cannot revoke data already read or synced. Use an all-bare list or enter recipients manually in Mail to avoid this named-list clipboard path.",
            ...
            Tool(
                name: "update_draft",
                description: "Replace an existing draft (upsert): locate it by draft_id (from list_drafts) or an EXACT subject_match, create the replacement via the same mechanism and eligibility rules as create_draft, then delete the old draft. A draft_id is a transient message ROWID: autosave or synchronization can change it between list_drafts and this call. On a zero-match id, re-list in the same account scope (if this call was unscoped, choose the intended account with list_accounts first); use subject_match only when the exact subject is unchanged and unique there. There is no automatic id-to-subject retry, and subject_match is not a stable identity key. Order is deliberately create-THEN-delete with a post-create receipt — the failure direction is always toward keeping drafts (worst case both MAY exist, recoverable), never toward losing both; this is the reverse of the naive delete-first flow, chosen for data safety. Ambiguity always refuses: 0 matches (update requires an existing draft — use create_draft for a new one) or >1 matches (candidates {id, subject} are listed; retry with draft_id). Notes: Apple Mail drafts cannot be edited in place, so the replacement is a NEW draft with a NEW id (never reuse the old id); the replacement is created under create_draft's account semantics (default account unless from_address) which may differ from the old draft's account; the body inherits create_draft's refusal contract (#304) — the replacement is refused, and the OLD DRAFT LEFT UNTOUCHED, for any of the six reasons create_draft lists. Non-ASCII attachment paths are refused before creating the replacement; create a new draft without attachments and drag files into it manually instead. On a draft a custom from_address (#219) and display-name to/cc/bcc recipients (#277/#404 — AX-addressed GUI fill with token read-back; a hidden Bcc field is revealed and left visible, reported as bcc_field_revealed: true) are supported when Accessibility is granted, and the replacement's cc/bcc addresses are re-read after the save (recipients_verified: true|false + recipients_diff in new_draft; the replacement is kept either way; true means the draft the receipt selected by subject — newest id wins, #409 — carried the requested addresses, so on a same-subject update it MAY describe the old draft). Draft scans and the final id+subject deletion run through cancellable osascript subprocesses with a 45-second per-script deadline (#406). Both the recipient receipt and post-create ID receipt stop on the first read failure without retry; only successful reads that have not found the target are polled (at most three reads, 0.4 seconds apart). An unavailable post-create ID receipt keeps the old draft and asks you to inspect Mail before retrying the update. A later macOS 27 live check (#406) covered display-name cc/bcc creation and update with Bcc already visible. Contacts-renamed token behavior and field identifiers on other macOS versions (including macOS 13–26) still require live verification (#408). Deletion moves the old draft to Trash (recoverable). The delete is gated three ways: (a) a post-create RECEIPT re-lists the drafts and requires a NEW id to appear before the old draft is touched (a GUI-path phantom success keeps the old draft, reported as deleted_old:false / not confirmed; under #405 id drift a re-saved OLD draft can also present a new id, so a new id is evidence, not proof, that the replacement exists); (b) when cc/bcc carried a display name (the receipt runs only then), the post-save recipient receipt must not report a definitive mismatch — if it does, the old draft is KEPT and deleted_old:false carries a note that says two drafts MAY exist and that the receipt identifies a draft by subject only (#409), never an instruction to delete; a receipt that could not run (recipients_receipt: unavailable, #406) does not block the delete; (c) the delete predicate conjoins id AND exact subject, so a cross-account id collision cannot delete another account's draft. If deletion fails after a confirmed create, the result reports deleted_old:false with a note matched to what is known (confirmed absent / state unknown / both MAY exist) — nothing is silently lost or over-claimed. If a To/Cc/Bcc list contains any display name, that entire list (including bare entries and Bcc addresses) is temporarily written to the system clipboard for GUI filling. Clipboard-reading apps or Universal Clipboard may obtain it depending on system settings. Restoring prior clipboard contents cannot revoke data already read or synced. Use an all-bare list or enter recipients manually in Mail to avoid this named-list clipboard path.",
                inputSchema: .object([
                    "type": .string("object"),
                    "properties": .object([
                        "draft_id": .object(["type": .string("string"), "description": .string("Current numeric message ROWID from list_drafts, not a stable key: autosave or synchronization may replace it before this call. Re-list in the same account scope after a zero match, or use subject_match for an unchanged unique subject. Exactly one of draft_id / subject_match is required.")]),
                        "subject_match": .object(["type": .string("string"), "description": .string("EXACT subject equality match (never substring/fuzzy — misfires would delete the wrong draft; must be non-empty — target an empty-subject draft via draft_id). Refuses when 0 or >1 drafts match. Exactly one of draft_id / subject_match is required. This content selector is not a stable identity key: the subject must remain unchanged and unique in the chosen account scope.")]),
            ...
            Tool(
                name: "open_mailto",
                description: "Open a mailto URL in the system default mail client via LaunchServices — ZERO Automation TCC required (#287), so it works even when AppleScript tools fail with -1743 (Not authorized to send Apple events). The mailto compose window is inherently cite-block-free (#175). Available clean compose routes: create_draft needs Automation + Accessibility TCC and supports eligible attachments; THIS TOOL needs zero TCC, has no attachments (RFC 6068; drag files in manually), and opens the default mail app which may not be Mail.app. Save or send manually. When Automation is denied (-1743) or Accessibility is unavailable, use this tool instead of a GUI-driven composing call.",
                inputSchema: .object([
                    "type": .string("object"),
                    "properties": .object([
    ...
    private func executeToolCall(name: String, arguments: [String: Value]) async throws -> String {
        ...
        case "check_accessibility":
            ...
            return AccessibilityStatus.report(for: AccessibilityStatus.probe())
