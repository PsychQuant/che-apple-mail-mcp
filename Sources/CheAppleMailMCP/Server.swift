...
class CheAppleMailMCPServer {
    ...
    static func defineTools() -> [Tool] {
        [
            ...
            Tool(
                name: "compose_email",
                description: composeSignatureToolDescription + " " + "MAILTO_URL_TOO_LONG refuses encoded URLs above 8000 before Mail work; check_compose_length reports exact body/total size. Save the full body to a local text file and manually paste in Mail, or use separately approved smaller messages. Compose and send a new email. The body always comes from Mail's own editor, never from an AppleScript body assignment — that assignment triggered the reported upstream regression FB11734014, wrapping the whole letter in <blockquote type=\"cite\">, which the sender cannot see locally while Gmail and Outlook show it as quoted text (#304). When the clean path cannot run, the call FAILS with the reason and an alternative and creates/sends nothing. Seven named input/eligibility reasons: encoded mailto URL above 8000 (MAILTO_URL_TOO_LONG); format is not 'plain'; empty subject; Accessibility not granted (grant it, or use open_mailto — zero TCC, no attachments); from_address is not a bare addr-spec; an attachment path contains non-ASCII characters (create the draft without attachments and drag the file in, #220); a to/cc/bcc recipient carries a display name on this SEND (use bare addresses, or create_draft — where display names in to/cc/bcc are supported, #277/#404 — and send from Mail). If the clean path fails AT the send-keystroke stage the tool does NOT retry (the mail may already be sent — a retry could send a duplicate): it returns an unknown-send-state error telling you to check Sent/Outbox before re-sending (#242).",

                inputSchema: .object([
                    "type": .string("object"),
                    "properties": .object([
                        "signature": ComposeSignatureSelection.schema,
                        ...
                        "format": .object(["type": .string("string"), "enum": .array([.string("plain")]), "description": .string("Body format. \"plain\" is the ONLY supported value and the default when omitted: the body is delivered verbatim through Mail's own editor. \"markdown\" and \"html\" were removed in #304 — no path this server ships today can deliver rich text without assigning the AppleScript html-content property, and that triggered the reported upstream regression FB11734014, wrapping the whole letter in <blockquote type=\"cite\"> (invisible to the sender, shown as quoted text by Gmail and Outlook). This is a product integration limit, not proof of impossibility: #306 reports rich-paste drafts and HTML-combination Sent/received MIME passing. Those ASCII results do not establish other OS versions, CJK, or sending with every flavor (#310). Passing either value fails with that explanation; see #308 / #309 for alternative rich-text architectures.")]),
            ...
            Tool(
                name: "reply_email",
                description: "Reply to an email. Mail's native reply verb builds the quoted original (correct threading headers and cite block) and only the NEW body is pasted in at the cursor. The body always comes from Mail's own editor, never from an AppleScript body assignment — that assignment triggered the reported upstream regression FB11734014, wrapping the whole letter in <blockquote type=\"cite\">, which the sender cannot see locally while Gmail and Outlook show it as quoted text (#304). When the clean path cannot run, the call FAILS with the reason and an alternative and creates/sends nothing. Two reasons apply here: format is not 'plain'; Accessibility not granted. Optionally add extra CC, attach files, and save as draft instead of sending.",
            ...
            Tool(
                name: "forward_email",
                description: "Forward an email. Mail's native forward verb builds the quoted original and only the NEW body is pasted in at the cursor. Body is optional — omit it for a bare forward, which assigns nothing at all and therefore needs no Accessibility grant. The body always comes from Mail's own editor, never from an AppleScript body assignment — that assignment triggered the reported upstream regression FB11734014, wrapping the whole letter in <blockquote type=\"cite\">, which the sender cannot see locally while Gmail and Outlook show it as quoted text (#304). When the clean path cannot run, the call FAILS with the reason and an alternative and creates/sends nothing. When a body IS provided, exactly Two reasons can refuse the call: format is not 'plain'; Accessibility not granted.",
        ...
        ] + classificationToolDefinitions() + [checkComposeLengthTool]
    }
    ...
    func executeToolCall(name: String, arguments: [String: Value]) async throws -> String {
        ...
        switch name {
        case "check_compose_length":
            guard let to = try optionalStringArray(arguments, key: "to"),
                  let subject = arguments["subject"]?.stringValue, let body = arguments["body"]?.stringValue else {
                throw MailError.invalidParameter("to, subject and body are required")
            }
            let report = composeLengthPreflight(to: to, subject: subject, body: body,
                cc: try optionalStringArray(arguments, key: "cc"), bcc: try optionalStringArray(arguments, key: "bcc"))
        ...
        // Compose Tools
        case "compose_email":
            let signature = try ComposeSignatureSelection.parse(arguments["signature"])
            ...
            return try await mailController.composeEmail(to: to, subject: subject, body: body, cc: cc, bcc: bcc, attachments: attachments, format: format, fromAddress: fromAddress, signature: signature)
        ...
        case "create_draft":
            ...
            let fromAddress = arguments["from_address"]?.stringValue
            return try await mailController.createDraft(to: to, subject: subject, body: body, cc: cc, bcc: bcc, attachments: attachments, format: format, fromAddress: fromAddress, signature: signature)
