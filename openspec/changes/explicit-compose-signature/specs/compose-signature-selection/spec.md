...
#### Scenario: Native menu tracking blocks scripting
- **WHEN** a signature menu is open and Mail does not service native scripting requests
- **THEN** selection SHALL use the captured AX window identity, with native identity checks before opening and after closing the menu, rather than deadlock on an in-menu Mail scripting query

#### Scenario: Verification fails while a menu is open
- **WHEN** a menu read, identity check, or selection verification fails after opening
- **THEN** cleanup SHALL confirm the captured AX owner, cancel only that popup's menu and confirm its disappearance before native Mail queries; otherwise it SHALL report the retained window without native cleanup or a global Escape
