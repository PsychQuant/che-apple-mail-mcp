...
The additional `is_draft` field SHALL follow the nullable per-message draft contract and describe the selected row.
...
- **THEN** the response is an envelope `{ results, returned, limit, truncated }` whose `results` elements each contain exactly `id`, `date`, `sender`, `subject`, `mailbox`, and `is_draft`
...
The system SHALL provide email metadata (read status, flagged status, size, deleted status, conversation_id) by querying the `messages` table directly. When this SQLite path is unavailable, the existing AppleScript fallback SHALL report is_draft as null; it SHALL NOT infer a draft flag from mailbox names.

#### Scenario: Get metadata for a message

- **WHEN** `get_email_metadata` is called with a message `id`
- **THEN** the system queries the `messages` table for the row with that ROWID and returns read, flagged, deleted, size, date_received and nullable is_draft fields
