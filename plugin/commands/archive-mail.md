...
**安全不變量**。這一節先定義不變量（不可協商），再給預設程序（可被格式慣例取代）。

> **這個保證的範圍很窄，先講清楚。** `safe` 只保證**當成一個路徑片段是安全的**。它
> **不**保證可以安全嵌進 Markdown（見「Markdown 顯示文字」）、**更不**保證可以安全嵌進
> shell 原始碼（見下方鐵律）。#395 verify round 3 抓到的兩個缺陷都源自把這三種「安全」
> 當成同一件事。

#### 安全不變量（三條，任何產生路徑片段的規則都必須滿足）

**I1.** `safe` 不得含 `/`、`\`，或任何 C0/C1 控制字元與 DEL。
**I2.** `safe` 去除首尾空白後，**不得等於** `.` 或 `..`。
**I3.** `safe` 不得為空。

**檢查點在最後，不是在中間。** 所有轉換、截斷、fallback **全部做完之後**，再驗一次 I1–I3；
任一條不成立就改用 `unnamed`。round 3 的 CRITICAL 正是死在這裡：舊版靠「移除前導 `.`」
**間接**排除 `..`，但那一步排在 trim 之前，於是 `" ../"`（前面一個空白）走完全程會產出精確的
`..` —— 已機械複驗。**間接排除不算排除**；I2 必須被直接、最後檢查。
...
2. 移除路徑分隔符 `/` 與 `\`（**移除，不是替換**）。
3. **先**去除首尾空白。（順序關鍵，見上方 I2 的說明。）
4. 移除**所有前導** `.`，然後再次去除首尾空白。
   **注意**：`..` 作為**子字串**不觸發任何拒絕 —— `a..b.png` 是合法檔名，照用。
5. 截斷到 **200 bytes**（UTF-8，不切破 code point），再次去除首尾空白。
6. 結果為空 → `safe = fallback_seed`，且 **`fallback_seed` 自己也必須走完第 1–5 步**
   （round 1 抓到的缺陷：早期版本 fallback 到未消毒的 `{cid}.png`，等於在防線上開旁路）。
...
Step 5.1 的 subject→檔名規則（標點轉 `-`、50 grapheme 截斷、`no-subject` fallback、
`Re--` 連續 dash 保留）**刻意不與上面的預設程序合併** —— 那些是為了與既有歸檔檔名相容的
**格式政策**，強行統一會改掉已經寫在磁碟上的檔名。它們的義務是：**跑完自己的規則後，
一樣要過 I1–I3 的最終檢查**。

> round 3 的更正：本節前一版寫「本 SOP 只有這一份消毒規格；Step 5 / 5.5.0 / 5.5.1 都引用它，
> 不得各自另立規則」。**那句話是假的** —— Step 5.1 從來就有自己一套七步規則，方向甚至相反
> （它把前導 `.` 加底線前綴，這裡是移除）。三份分岔規格收斂成一份，實際只做了三分之二，
> 而文件寫成已完成。現在改成誠實的兩層結構：不變量共用，格式各自。

#### `safe` 只用於組路徑

傳給 `save_attachment(attachment_name=...)` 的查找鍵一律用**未消毒的原始值** —— 那是 Mail 端
的物件名稱，消毒它會查不到附件。兩者是不同用途，**必須是不同變數**，不要共用一個名字。
...
`safe` 對 Markdown **沒有**任何保證：I1–I3 不碰 `]`、`[`、`(`、`)`、`!`。一個名為
...
結果加入 corpus（與 recipe (1) 等地位，見 3d 的四組合併）。

> **Bcc 注意**：`field: "recipient"` 比對 recipients 表、不分 To/Cc/Bcc；但 **Bcc 是否被 Envelope Index 收錄無法保證**。若任務需涵蓋 Bcc-only 寄件，直接稽核 Sent 匣（unscoped + keep-only-Sent）或人工確認。

> **Tier 2 兜底（scoped 回 0 筆 → 一律執行）**：`mailbox` 參數在 SQLite 層以 percent-encoding 比對 mailbox URL，CJK / `[Gmail]` 括號路徑有編碼不匹配的已知風險。**scoped 搜尋回 0 筆就一律**退回 **unscoped** 搜尋（同上但不帶 `mailbox`）——fallback 經 Message-ID 去重、冪等，寧可多跑——再對結果 `mailbox` 欄位以上方**比對規則**判定：**只保留** Sent。Tier 2 也回 0 筆才記錄為「該帳號無寄出信」。兩層結果都以 Message-ID 去重收斂（見下方紀律 3）。
...
結果同樣套上方 **mailbox 後過濾規則**的統一 drop-set（subject 搜尋跨匣，草稿同樣會混進來，plugins#109）。
...
**四組結果**（sender + **recipient/sent** + subject_keywords + thread-subject）用 Message-ID 去重。最終 corpus 進入 Step 4。

在 Step 7 報告中加上：`搜尋結果: {sender_count} by sender + {sent_count} by recipient/sent + {keyword_count} by subject + {thread_count} by thread expansion = {total_unique} unique`
...
Subject → filename 轉換規則（依此順序執行）。**這是格式政策，不是安全規格** —— 跑完之後
必須過「Safe leaf filename」的 I1–I3 最終檢查（#395 verify round 3；那一節說明為什麼這兩層
刻意不合併）：

0. **先移除 C0/C1 控制字元與 DEL**（本步為 round 3 新增：原規則七步完全沒有這一項，而
   subject 來自郵件、可含 `\n` `\t` `NUL`）
...
8. **最終不變量檢查**：驗「Safe leaf filename」的 I1/I2/I3。任一條不成立 → `no-subject`。
   （步驟 2 的「`.` 開頭加 `_` 前綴」已使 `.` / `..` 這兩個 subject 產出 `_.` / `_..`，
   本身滿足 I2；本步是為了讓保證來自**直接檢查**而非某條規則的副作用 —— round 3 的
   CRITICAL 正是死在「間接排除」上。）
...
**Frontmatter 欄位說明**：
- `message_id`: 該封信的 RFC 5322 Message-ID（用引號包住，避免 YAML 解析角括號）。**缺值規則（mail#319，與 `in_reply_to` 同級的明文 fallback）**：工具回傳空 message_id 時，**先**呼叫 `get_email_headers(id: 同一id, mailbox: 原匣, account_name: 原帳號)` 重取一次（headers 路徑與 body 路徑不同 code path，常能拿到值）；仍空 → frontmatter 寫 `message_id: ""` 並加一行 `message_id_missing: true`，讓 Step 8.5 走 `unparseable` 分支自然浮現。**明文禁止發明任何佔位符**（`synthetic:<timestamp>` 或其他）——synthetic key 不是真 Message-ID，無法可靠對齊其他 target 或重取的真 ID。mail#389 已觀察到以信件日期、寄件人及截斷主旨產生的 deterministic 格式；同一 target 內可能穩定去重，但仍有截斷碰撞風險。只有使用執行時間的歷史變體才會每次重跑產生不同 key，不能概括所有 synthetic 格式。既有 synthetic 檔的一次性修復見 `/archive-mail-repair-synthetic-ids`
...
1. 用 bare subject 搜尋，同時取回每筆 `mailbox`（套 drop-set 用）並在伺服器端做邏輯去重（對齊分子的 Message-ID 去重基礎）：
   ...
   —— **必須 `projection: "summary"`**（唯一同時回 `mailbox` 又支援 `dedup` 的 projection：`count` 無 row 無法套 drop-set、`full` 不支援 `dedup`，plugins#208）；**`dedup: "logical"`** 在伺服器端塌陷同一封信的多匣副本（Gmail INBOX + 全部郵件 等）——否則含 dual-stored 的信會被重複計數，即使**零草稿**也 `archived < total` 每次誤報（plugins#112 的真正根因，不只是草稿）。`N` 取遠大於該 subject 預期兄弟數（**預設 limit 僅 50，通常不足**）。`dedup: logical` 依 (subject, sender, date_received) 分組，是分子 Message-ID 基礎的**近似**（勿宣稱逐 Message-ID 對等）。誠實揭露方向：此粗粒度 dedup 對 completeness 稽核偏「**少算**」而非「多算」——極罕見情形下兩封**真正不同**的信共用相同 subject+sender+同秒收信時戳會塌成一筆，若其中一封正是缺失的兄弟，`archived < total` 就不觸發而**漏報**（不是誤報）。正常郵件流下此三元組完全碰撞機率可忽略，且 `search_emails` 無 Message-ID 級 dedup，此為工具限制、非本步引入。
2. **對結果每筆的 `mailbox` 套用 Step 3 的統一 drop-set**（丟 Drafts / Trash 實名 + `exclude_mailboxes`，比對規則見 Step 3 的 `matchesSpecial`，**勿裸 `==`**），數過濾後的存活筆數為 total —— 分子（已歸檔 count）已經 Step 3 Message-ID 去重且排除草稿/垃圾桶；分母同時經 `dedup: logical` 去重（步驟 1）＋ drop-set 排除（本步），兩邊才同基礎。
   > **實作註（drop-set 套在代表匣上）**：`dedup: logical` 去重後每筆只剩一個代表 `mailbox`，drop-set 即套在該代表匣。此在 Gmail 模型下安全——dual-storage 對（INBOX/全部郵件、寄件備份/全部郵件）全是**非 drop** 匣，而 Drafts/Trash 副本因 sender/date 不同不會與收件塌成同一筆，故代表匣不會誤把該保留的信丟掉。非 Gmail provider 若存在「同一信橫跨 kept + dropped 匣」的儲存模型，需另行驗證代表匣選取不會漏丟。
