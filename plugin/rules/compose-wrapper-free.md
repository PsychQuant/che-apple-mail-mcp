<!-- Imported from the server repo's .claude/rules/compose-wrapper-free.md
     ...
     shell v2.43.0; re-synced for #304. -->
...
| 6 | 收件人帶顯示名（`Name <addr>`）| cc/bcc 一律用純位址；`to` 的顯示名**只有 `create_draft` 支援**（GUI 填入，#277）|
...
2. **`compose_email` 直接寄給 `Name <addr>`**。以前靠 legacy 原生設定收件人姓名（代價是 body 被 wrap）。
   乾淨路徑的 GUI 填入是**草稿限定**（送信時填入失敗會漏收件人），所以現在直接寄會被拒。
   要保留人名 → 用 `create_draft` 建草稿、確認 To 後手動寄出。
...
- `#219` — 自訂寄件人；`#277` — 草稿的顯示名 To；`#220` — CJK 附件路徑
